<g:if test="${item?.status=='GOOD'}"><span class="item-status-show label label-success">${item?.status}</span></g:if>
<g:elseif test="${item?.status=='BAD'}"><span class="item-status-show label label-danger">${item?.status}</span></g:elseif>
    ...
    $("#item-status-save").click(function(){
        ...
        $.ajax({ url: "/pegr/item/updateStatusAjax?itemId=${item.id}&status=" + status,
            success: function(result) {
                $("#item-status-show").text(result);
                $("#item-status-select").val(result);
