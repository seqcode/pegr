<h4>Protocol group: ${it.name} ${it.id}</h4>
<ol class="list-group">
	<g:each in="${it.protocols}" var="protocol" status="count">
		<g:if test="${count < protInstCount}">
		<li class="list-group-item list-group-item-info">
		<g:link controller="protocol" action="editForSample" id="${protocolInstances[count].id}" params="[sampleId: sampleId]">
		...
		<g:if test="${count == protInstCount}">
			<g:if test="${count >0}">
			<g:link controller="protocol" action="createForSample" 
