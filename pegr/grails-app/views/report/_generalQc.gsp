...
<div id="qc-statistics">
    <table class="table">
        ...
        <g:each in="${runStatusMap.value.sampleStatusList}" var="sample">
            <tr>
            <td class="col-sample group-analysis" rowspan="${Math.max(1, sample.analysisStatusList.size())}"><g:link controller="sample" action="show" id="${sample.sampleId}">${sample.sampleId}</g:link> ${sample.naturalId}</td>
            <td class="col-target group-analysis" rowspan="${Math.max(1, sample.analysisStatusList.size())}">${sample.target}</td>
            <td class="col-cohort group-analysis" rowspan="${Math.max(1, sample.analysisStatusList.size())}">${sample.cohort}</td>
            <g:each in="${sample.analysisStatusList}" var="analysisWorkflowRun" status="n">
                <g:if test="${n>0}"><tr></g:if>
                    <td class="col-genome group-analysis">${analysisWorkflowRun.genome}</td>
                    <td class="col-history group-analysis">
                        <g:if test="${analysisWorkflowRun.historyUrl}"><a href="${analysisWorkflowRun.historyUrl}" target="_blank">${analysisWorkflowRun.historyId}</a></g:if><g:else>${analysisWorkflowRun.historyId}</g:else>
                </td>
                    <td class="col-date group-analysis">${analysisWorkflowRun.date}</td>
                    <g:each in="${analysisWorkflowRun.status}" var="status" status="j">
                    ...
                    <td class="text-right col-tags group-qc"><g:formatNumber number="${analysisWorkflowRun.requestedTags}" format="###,###,###" /></td>
                    <g:each in="${qcSettings.general}" var="setting">
                      <g:if test="${setting.name != "Deduplicated"}">
                        <td class="text-right col-${setting.key} group-qc <g:if test='${
                                   (setting.min != null && analysisWorkflowRun[setting.key] < setting.min)
                                   || (setting.max != null && analysisWorkflowRun[setting.key] > setting.max)
                                   || (setting.reference_min != null && analysisWorkflowRun.hasProperty(setting.reference_min) && analysisWorkflowRun[setting.key] < analysisWorkflowRun[setting.reference_min] * setting.reference_min_ratio)
                                   || (setting.reference_max != null && analysisWorkflowRun.hasProperty(setting.reference_max) && analysisWorkflowRun[setting.key] > analysisWorkflowRun[setting.reference_max] * setting.reference_max_ratio)
                                   }'>bg-danger</g:if>">
                        <g:formatNumber number="${analysisWorkflowRun[setting.key]}" format="${setting.numFormat}" />
                        </td>
                      </g:if>
                    </g:each>
                    <td class="col-prefer group-operation">
                        <span class="analysisWorkflowRunId" style="display:none">${analysisWorkflowRun.analysisWorkflowRunId}</span>
                        <label class="switch">
                            <input class="prefer" type="checkbox" <g:if test="${analysisWorkflowRun.isPreferred}">checked</g:if>>
                            <div class="slider round"></div>
                        </label>
                    </td>
                    <td class="col-delete group-operation"><input type="checkbox" name="delete" value="${analysisWorkflowRun.analysisWorkflowRunId}" data-runId="${run.id}"></td>
                </tr>
            </g:each>
            <g:if test="${sample.analysisStatusList.size()==0}">
                </tr>
            </g:if>
        </g:each>
        </tbody>
...
<script type="text/javascript">
	...
	$("#ajaxDeleteAll").click(function() {
		var analysisWorkflowRunIds = [];
		var runId = 0;
		$('input[name="delete"]').each(function(){
			if (this.checked) {
				analysisWorkflowRunIds.push(this.value);
				runId = this.getAttribute("data-runId");
			}
		});
		if (confirm('All data in the selected analysis workflow run(s) will be deleted. Are you sure you want to delete the following analysis workflow run(s): ' + analysisWorkflowRunIds + ' for run number: ' + runId + '?'))
		{ // axa677-180306: the next ajax call sends the array as a json dictionary with the run id to a controller action
		  // then get the results as html
			$.ajax({
				...
				type:"GET",
				data: {"analysisRunIdsList": JSON.stringify(analysisWorkflowRunIds), "runId": runId},
